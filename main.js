...
class RPS {
  ...
  updateScore() {
    document.getElementById(
      "score"
    ).textContent = `${this.score[0]}/${this.score[1]}`;
  }
  generateWeapon() {
  ...
  main([user, bot]) {
    let result = this.roundAlgo(user, bot);
    this.showSceneTwo([user, bot, result])
    console.clear();
    console.log(`user chose: ${this.weapons[user]}`);
    console.log(`bot chose: ${this.weapons[bot]}`);
  ...
  showSceneTwo([user, bot, result]) {
    var elem = document.getElementById("scene2")
    elem.classList.add("shown");
    document.getElementById("scene1").classList.remove("shown");
    elem.querySelector(".result").querySelector("span").innerText = result === 1 ? "WIN" : (result === 0 ? "DRAW" : "LOSS")
    elem.querySelectorAll(".weapon").forEach((itm, idx)=> {
      itm.classList.add(this.weapons[idx === 0 ? user : bot].toLowerCase())
      itm.querySelector("span").innerHTML = this.SVG[idx === 0 ? user : bot]
    })
    
    let ts = setTimeout(() => {
        this.hideSceneTwo()
        elem.querySelectorAll(".weapon").forEach((itm, idx)=> {
          itm.className = "weapon"
        })
    }, 2000);
  }
  hideSceneTwo() {
    document.getElementById("scene2").classList.remove("shown");
    document.getElementById("scene1").classList.add("shown");
  }
}
const RPS_game = new RPS();